...
def place_objects(poly:Polygon, objects:list[Polygon])->tuple[bool, list[Polygon]]:
    ...
    def _objective_function(args):
        ...
        return distance(poly, o, penalty) 
...
def distance(poly:Polygon | MultiPolygon, obj: Polygon, penalty:float)->float:
    ...
    for i in range(len(p_ext)):
        if i > 0 and i<len(p_ext)-1 and (p_ext[i+1][0] - p_ext[i-1][0]) * (p_ext[i][1] - p_ext[i-1][1])\
            ...
            # оно необходимо, чтобы не объекты не магнитились к местам скопления точек полигона
    ...
    #penalty = 0, а так же максимизируем обратную функцию расстояния
    return (poly.union(obj).area - poly.area) - dist + penalty
